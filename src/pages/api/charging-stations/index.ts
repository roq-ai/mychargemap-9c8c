...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getChargingStations() {
    const data = await prisma.charging_station
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'charging_station'));
    return res.status(200).json(data);
  }

  async function createChargingStation() {
    ...
    const data = await prisma.charging_station.create({
      data: body,
    });
    return res.status(200).json(data);
  }
