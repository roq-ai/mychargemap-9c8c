...
function ReviewListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ReviewInterface[]>(
    ...
    () =>
      getReviews({
        relations: ['user', 'charging_station'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('charging_station', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('charging_station', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/charging-stations/view/${record.charging_station?.id}`}>
                          {record.charging_station?.location}
                        </Link>
                      </Td>
                    )}
